...
router.get('/config', async (ctx, next) => {
  ...
  for (let index = 0; index < configs.length; index++) {
    let { name, fileName } = configs[index];
    if (type === name) {
      // request link
...
router.get('/proxy/:type', async (ctx, next) => {
  // get params type
  let type = ctx.params.type;

  // request setting
  let { proxies } = await getSetting();

  // get matched proxy
  for (let index = 0; index < proxies.length; index++) {
    let { fileName, link } = proxies[index];
    if (type === fileName) {
      // request link
...
router.get('/server', async (ctx, next) => {
  ...
  for (let index = 0; index < servers.length; index++) {
    ...
    if (type === server.name) {
      ...
      let extra = 'extra' in server ? server.extra : [];
