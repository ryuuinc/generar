...
const { packPromise, removeUselessTag, filterMatchedNode } = require('./utils');
...
router.get('/server/:key', async (ctx, next) => {
  ...
  for (let index = 0; index < servers.length; index++) {
    let { key, link, fileName, withNode } = servers[index];
    if (key === ctx.params.key) {
      ...
      if (result != undefined) {
        ...
        // remove useless tag
        let proxies = removeUselessTag(result.data);
        ...
        // filter matched node
        let filter = decodeURIComponent(ctx.query.filter);
        filter = filter.replace(/-/g, '|');
        let reg = new RegExp(filter);
        proxies = filterMatchedNode(proxies, reg);
