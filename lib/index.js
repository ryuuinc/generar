...
const getSetting = async () => {
  ...
  if (result != undefined) {
    ...
    const targetFile = files.find((file) => file.filename === process.env.SETTING_NAME)

    if (targetFile) {
      return JSON.parse(targetFile.content)
    } else {
      throw new Error(`未找到配置文件: ${process.env.SETTING_NAME}`)
    }
  }
}
  ...
  for (let index = 0; index < configs.length; index++) {
    ...
    if (key === ctx.params.key) {
      ...
      if (result != undefined) {
        ...
        // 新的 API 结构：files 是一个数组，需要根据文件名查找
        const files = result.data.files
